    ...
    <?php
      ...
      function approve() {
       ...
       /* if ($query1) 
        ...
        // If successful, fetch the updated data from the table with 'approved' status
        $sql2 = "SELECT * FROM leaverequests WHERE regno='$regno1' AND status='approved'";
        $query2 = mysqli_query($conn, $sql2);*
        	
        
         if ($query2) {
        // Generate an HTML table to display the updated data
      //  echo '<table border="1">';
        // ... (display rows)
        //echo '</table>';

        // Redirect to another page with URL parameters
        header("Location: security.php?regno=$regno1");
        exit(); // Ensure that no further output is sent
        
    } 
    else {
        echo "Error occurred while fetching updated data: " . mysqli_error($conn);
    }
    */
